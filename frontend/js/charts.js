/**
 * Visualizaciones con Chart.js: dona (gastos por categoría) y líneas
...
(function (global) {
  ...
  function _destruir(chart) {
    if (chart) {
      chart.destroy();
    }
  }

  // Gráfico de dona: gastos totales por categoría
  function renderizarDona(elemento, datos) {
    _destruir(donaChart);
    if (!elemento || !datos || datos.length === 0) {
      return;
    }
  ...
  function renderizarLineas(elemento, puntos) {
    _destruir(lineaChart);
    if (!elemento || !puntos || puntos.length === 0) {
      return;
    }
