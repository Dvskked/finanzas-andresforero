...
class Database:
    ...
    # ---------------- utilidades internas ----------------
    def _connect(self):
        return _mysql_connection() if self.db_type == "mysql" else _sqlite_connection()
...
def init_db():
    ...
    if config.db_type == "mysql":
        ...
        try:
            ...
            kwargs = _mysql_kwargs_sin_database()
            temp = mysql.connector.connect(**kwargs)
            cur = temp.cursor()
            cur.execute(
...
def _mysql_kwargs_sin_database():
    import copy
