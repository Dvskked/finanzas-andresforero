-- ============================================================
-- Seed Script - Finanzas Personales
-- Datos demo no duplicados (verifican existencia previa).
-- Contraseña de los usuarios demo: "demo1234"
-- ============================================================

-- Usuario demo ------------------------------------------------------------
INSERT INTO usuarios (nombre, email, contrasena_hash)
SELECT 'Usuario Demo', 'demo@finanzas.com',
       '$2b$12$ZLDmw4Ut6tXfoS34DoCB.enpAO5TlKImfwi9RR.aeLBhh0bcz4E0K'
WHERE NOT EXISTS (
...
-- Movimientos demo (solo si no hay ninguno) --------------------------------
...
FROM usuarios u JOIN categorias c ON c.nombre = 'Sueldo' AND c.tipo = 'ingreso'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);

INSERT INTO ingresos_gastos (usuario_id, categoria_id, tipo, monto, fecha, descripcion)
SELECT u.id, c.id, 'gasto', 320.00, '2026-01-03', 'Supermercado quincenal'
FROM usuarios u JOIN categorias c ON c.nombre = 'Alimentación' AND c.tipo = 'gasto'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);

INSERT INTO ingresos_gastos (usuario_id, categoria_id, tipo, monto, fecha, descripcion)
SELECT u.id, c.id, 'gasto', 150.00, '2026-01-10', 'Combustible'
FROM usuarios u JOIN categorias c ON c.nombre = 'Transporte' AND c.tipo = 'gasto'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);

INSERT INTO ingresos_gastos (usuario_id, categoria_id, tipo, monto, fecha, descripcion)
SELECT u.id, c.id, 'gasto', 800.00, '2026-01-01', 'Renta mensual'
FROM usuarios u JOIN categorias c ON c.nombre = 'Vivienda' AND c.tipo = 'gasto'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);
...
FROM usuarios u JOIN categorias c ON c.nombre = 'Entretenimiento' AND c.tipo = 'gasto'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);

INSERT INTO ingresos_gastos (usuario_id, categoria_id, tipo, monto, fecha, descripcion)
SELECT u.id, c.id, 'ingreso', 500.00, '2026-02-15', 'Proyecto freelance'
FROM usuarios u JOIN categorias c ON c.nombre = 'Freelance' AND c.tipo = 'ingreso'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);
...
FROM usuarios u JOIN categorias c ON c.nombre = 'Salud' AND c.tipo = 'gasto'
WHERE u.email = 'demo@finanzas.com' AND NOT EXISTS (SELECT 1 FROM ingresos_gastos);
